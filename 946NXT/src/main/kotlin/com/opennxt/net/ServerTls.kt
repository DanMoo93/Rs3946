...
object ServerTls {
    ...
    val available: Boolean by lazy { Files.exists(pfxPath) }

    val context: SslContext? by lazy {
        if (!Files.exists(pfxPath)) {
            logger.warn { "TLS certificate not found at $pfxPath — TLS disabled (plaintext only)" }
            return@lazy null
        }
    ...
    fun newHandler(ctx: ChannelHandlerContext): SslHandler? {
        val sslCtx = context ?: return null
