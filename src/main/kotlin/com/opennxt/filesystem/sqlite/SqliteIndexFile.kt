...
//        val stmt = putArchiveDataStmt
    ...
    fun getRaw(id: Int): ByteArray? {
        connection.prepareStatement("SELECT `DATA` FROM `cache` WHERE `KEY` = ?;").use { stmt ->
            ...
            stmt.executeQuery().use {
                ...
                return unwrapZlb(it.getBytes("DATA"))
            }
        }
    }
    ...
    companion object {
        private val ZLB_MAGIC = byteArrayOf(0x5A, 0x4C, 0x42) // "ZLB"

        /**
         * Unwraps NXT jcache ZLB (Zlib Block) container format.
         * Format: magic("ZLB", 3 bytes) + version(1 byte) + decompressedSize(4 bytes BE) + zlib_data
         * Returns the original data if it's not ZLB-wrapped.
         */
        fun unwrapZlb(data: ByteArray?): ByteArray? {
            if (data == null || data.size < 8) return data
            if (data[0] != ZLB_MAGIC[0] || data[1] != ZLB_MAGIC[1] || data[2] != ZLB_MAGIC[2]) return data

            val decompressedSize = ByteBuffer.wrap(data, 4, 4).int
